...
| [terminal-workspace](../terminal-workspace) | Focused product sample | `SwiftTUI`, `SwiftTUITerminal` (through the example-owned `TerminalWorkspace` module) | Tabs, splits, retained sessions, command palette actions, persisted workspace metadata | Terminal workspace app | Linux native gate builds debug and release. Focused SwiftPM tests run in `check:focused`. |
