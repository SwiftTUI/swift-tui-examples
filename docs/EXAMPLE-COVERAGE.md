...
| [layouts](../layouts) | Focused product sample and stress/regression sample | `SwiftTUI`, `SwiftTUIRuntime`, `SwiftTUICharts` | Layout catalog for stacks, frames, geometry, scrolling, overlays, shapes, matched geometry, and custom layouts | Terminal app | Linux framework-seam gate builds debug (release on tags). The complete `LayoutsTests` suite runs in the default Linux/macOS gate, including behavior/raster, catalog markers and `LayoutComparisonBoundsTests`. |
...
- `bun run check:linux` is the Linux framework-seam gate and the push/PR CI
  ...
  journeys, and — last, because it parks at the 0.15.1 pin and pays the
