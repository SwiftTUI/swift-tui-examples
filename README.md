...
example pins the current `0.13.1` release graph. Gallery uses matching
...
| [sextant](sextant) | Terminal | Miller-column browser and file previews through `SwiftTUITerminalView` / `TerminalProcessSession` | `swiftly run swift run --package-path sextant sextant` |
| [terminal-workspace](terminal-workspace) | Terminal | Terminal multiplexer built on `SwiftTUITerminalView`, with an example-owned workspace layer: tabs, splits, retained sessions, command-palette actions, and persisted layout metadata | `swiftly run swift run --package-path terminal-workspace terminal-workspace` |
...
| `SwiftTUITerminalView` | [sextant](sextant), [terminal-workspace](terminal-workspace) |
