...
The count is total plays, including the first, and controls both export and the
editor preview. Zero means forever; `)`, the shifted `0`, toggles this value.
Play starts at the first frame. A finite preview holds its final frame after
the last delay; changing the count stops the current preview. GIF stores
repeats after the first play, so export subtracts one from finite counts and
omits the loop extension for one play. Import adds the initial play back. APNG
already stores total plays. Existing project counts retain their meaning.
