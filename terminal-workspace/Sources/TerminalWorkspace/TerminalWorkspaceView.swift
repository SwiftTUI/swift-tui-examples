...
public struct TerminalWorkspaceView: View {
  ...
  private func workspaceSurface(size: CellSize) -> some View {
    ...
    .paletteCommand(
      name: "Focus pane left", description: "Alt+H / Alt+←",
      action: { workspace.focus(.left, within: contentSize(for: size)) }
    )
    .paletteCommand(
      name: "Focus pane down", description: "Alt+J / Alt+↓",
      action: { workspace.focus(.down, within: contentSize(for: size)) }
    )
    .paletteCommand(
      name: "Focus pane up", description: "Alt+K / Alt+↑",
      action: { workspace.focus(.up, within: contentSize(for: size)) }
    )
    .paletteCommand(
      name: "Focus pane right", description: "Alt+L / Alt+→",
      action: { workspace.focus(.right, within: contentSize(for: size)) }
    )
    .paletteCommand(
      name: "Split pane right", description: "Alt+V",
      action: { splitFocusedPane(axis: .horizontal) }
    )
    .paletteCommand(
      name: "Split pane down", description: "Alt+S",
      action: { splitFocusedPane(axis: .vertical) }
    )
    .paletteCommand(
      name: "New shell pane", description: "Alt+N",
      action: { splitFocusedPane(axis: .horizontal) }
    )
    .paletteCommand(name: "New shell tab", description: "Alt+T", action: appendShellTab)
    .paletteCommand(
      name: "Focus next pane", isEnabled: workspace.activePaneIDs.count > 1,
      action: { workspace.focusNextPane() }
    )
    .paletteCommand(
      name: "Focus previous pane", isEnabled: workspace.activePaneIDs.count > 1,
      action: { workspace.focusPreviousPane() }
    )
    .paletteCommand(
      name: "Next tab", isEnabled: workspace.tabs.count > 1,
      action: { selectAdjacentTab(by: 1) }
    )
    .paletteCommand(
      name: "Previous tab", isEnabled: workspace.tabs.count > 1,
      action: { selectAdjacentTab(by: -1) }
    )
    .paletteCommand(
      name: workspace.zoomedPaneID == nil ? "Zoom focused pane" : "Unzoom focused pane",
      description: "Alt+Z", action: { workspace.toggleZoom() }
    )
    .paletteCommand(
      name: "Close focused pane", description: "Alt+X",
      isEnabled: workspace.canCloseFocusedPane, action: closeFocusedPane
    )
    .paletteSheet("Workspace commands", isPresented: $showsCommandPalette)
...
struct TerminalWorkspacePaletteStyle: PaletteStyle {
  func makeBody(configuration: PaletteStyleConfiguration) -> some View {
    TerminalWorkspaceCommandPalette(configuration: configuration)
  }
}
...
private struct TerminalWorkspaceCommandPalette: View {
  ...
  private var matches: [PaletteStyleConfiguration.Command] {
    ...
    return configuration.commands.filter { command in
